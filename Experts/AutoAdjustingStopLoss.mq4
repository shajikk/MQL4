//+------------------------------------------------------------------+
//|                                        AutoAdjustingStopLoss.mq4 |
//|                                               Shaji Kunjumohamed |
//| Create an auto adjusting stop loss based on :                    |
//| (a) Risk which we are willing to put in                          |
//|     How much reward we are looking at based on % of funds.       |
//|     eg.  If we get stopped at we loose 2% of our funds, if we
//|     gain we get 4% of our funds.
//| (b) Lot size we selected                                         |
//+------------------------------------------------------------------+
...
// Reward percent.
extern int RewardPercent = 5;

/*

  Ctrl F will change curser into a cross hair.
  Go to ancher pomit press left mouse button drag 
  mouse to destination. Look at number a/b/c
  b will give numer of pips.
...
void OrderEntry(int direction) {  
    ...
    // Find out the account equity
    double Equity = AccountEquity();
    double RiskedAmount = Equity * RiskPercent * 0.01;
    double RewardAmount = Equity * RewardPercent * 0.01;

    // We need to set the stop loss 200 (RiskedAmount) dollers away :
    // need to know the lot size we are running &
    // how many dollers a pip we are running
    // 1 lot = 10 dollers a pip
    //bsl = Ask - (RiskedAmount / (LotSize * 10)) * pips;
    ...
    if (direction == 0) {
      ...
      if (UsePercentTakeProfit) {
        btp = Ask + (RewardAmount / (LotSize * 10)) * pips;
      } 
    ...
    if (direction == 1) {
      ...
      if (UsePercentStop) {
        ssl = Bid + (RiskedAmount / (LotSize * 10)) * pips;
      } 
      ...
      if (UsePercentTakeProfit) {
        stp = Bid - (RewardAmount / (LotSize * 10)) * pips;
      } 
